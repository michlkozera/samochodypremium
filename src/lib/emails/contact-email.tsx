...
export function buildContactEmailHtml(data: ContactEmailProps): string {
  const rows = [
    ...
    row('Zainteresowanie', data.scopes.length > 0 ? data.scopes.join(', ') : undefined),
  ].join('');
  ...
  <div style="max-width:640px;margin:0 auto;padding:32px 16px">
    ...
    <div style="background:#fff;padding:28px;border-bottom:1px solid #e4e4e7">
      <p style="margin:0;font-size:15px;color:#09090b;line-height:1.7">
        Otrzymano nowe zapytanie od <strong>${data.name}</strong>.
      </p>
    </div>
    ...
    <div style="background:#fff;padding:20px 28px 24px">
      ...
      <p style="margin:0;font-size:14px;color:#09090b;line-height:1.8;white-space:pre-wrap">${data.details}</p>
    </div>
    ...
    <div style="padding:24px 28px;text-align:center">
      <p style="margin:0;font-size:11px;color:#a1a1aa;line-height:1.6">
        Wiadomość wysłana automatycznie z formularza kontaktowego na stronie samochodypremium.pl
      </p>
    </div>
...
export function buildContactEmailText(data: ContactEmailProps): string {
  const lines = [
    'NOWE ZAPYTANIE KONTAKTOWE — SAMOCHODY PREMIUM',
    '==============================================',
    '',
  ...
  if (data.scopes.length > 0) lines.push(`Zainteresowanie: ${data.scopes.join(', ')}`);
  lines.push('', 'WIADOMOŚĆ:', data.details);
  return lines.join('\n');
}
